...
- GHCR：给有网环境直接 `docker pull`
- GitHub Release：给离线环境提前准备镜像包和现场文档

推荐理解方式：

- 有网研发环境可走 GHCR
- 现场交付和内网用户默认走 Release 附件
...
- 源码仓库内容
- Release Notes
- 附件 `qwen-code-dev-0.13.4.tar.gz`
- 附件 `FIELD_USAGE.md`
...
```bash
cd release
shasum -a 256 qwen-code-dev-0.13.4.tar.gz > checksums.txt
```

## 4. 打 Git 标签

```bash
git tag -a v0.13.4 -m "Release v0.13.4"
...
```bash
ghcr.io/wzfukui/qwen-code-dev-container:0.13.4
...
- 从已发布镜像生成离线包 `qwen-code-dev-0.13.4.tar.gz`
- 生成 `checksums.txt`
...
gh release create v0.13.4 \
  ...
  /path/to/release/qwen-code-dev-0.13.4.tar.gz \
...
如果面对的是现场实施或客户环境，建议由交付方提前准备好 Release 附件，通过离线介质拷贝到现场，而不是让现场机器自己下载或构建。
